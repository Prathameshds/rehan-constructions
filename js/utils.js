...
const Utils = {
    ...
    showLoading: (element, message = 'Loading...', colspan = 15) => {
        if (element) $(element).html(`<tr><td colspan="${colspan}" class="loading-cell"><i class="fas fa-spinner fa-spin"></i> ${message}</td></tr>`);
    },

    showError: (element, message = 'Error loading data', colspan = 15) => {
        if (element) $(element).html(`<tr><td colspan="${colspan}" class="loading-cell text-danger"><i class="fas fa-exclamation-circle"></i> ${message}</td></tr>`);
    },

    showEmpty: (element, message = 'No data found', colspan = 15) => {
        if (element) $(element).html(`<tr><td colspan="${colspan}" class="loading-cell text-muted"><i class="fas fa-inbox"></i> ${message}</td></tr>`);
    }
};