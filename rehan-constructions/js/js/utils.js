...
const Utils = {
    ...
    showLoading: (element, message = 'Loading...') => {
        if (element) {
            element.innerHTML = `<tr><td colspan="100%" class="loading">${message}</td></tr>`;
        }
    },
    ...
    showError: (element, message = 'Error loading data') => {
        if (element) {
            element.innerHTML = `<tr><td colspan="100%" class="loading" style="color: var(--danger);">${message}</td></tr>`;
        }
    },

    // Show empty state
    showEmpty: (element, message = 'No data found') => {
        if (element) {
            element.innerHTML = `<tr><td colspan="100%" class="loading">${message}</td></tr>`;
        }
    }
